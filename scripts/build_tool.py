...
/* ---------- 解説の部品 ---------- */
function ex(title, body){
  return '<details class="ex" open><summary>'+title+'</summary><div class="body">'+body+'</div></details>';
}
...
const IDX={
  pe:{n:"自分基準で動くか、状況で動くか",t:"中点スコアの差（P群−E群）",u:"（0が分かれ目、±1.5未満はほぼ同程度）",
    ...
    r:v=> Math.abs(v)<1.5 ? "自分の基準と場の条件が、ほぼ同じくらいの強さで働いています。どちらか一方に絞らず、両方の打ち手を試してください。"
...
function theorySources(){
  ...
  ex("借りた部分と、このツールが作った部分",
    '<p>診断を受けたあと気になるのは「これはどこまで裏づけがあるのか」でしょう。'+
    'レヴィン本人の研究に由来する部分と、本ツールが作成した部分を分けて示します。</p>')+
  ...
  '</tbody></table>'+
  '<h3>この診断が測れていないこと</h3>'+
  '<p>改善を重ねても、実装では解消できない限界が2つあります。</p><ul>'+
  ...
  '<p>この2点がある限り、本ツールは測定器ではなく、'+
  '<b>構造化された振り返りの道具</b>として使うのが適切です。</p>'+
  '<div class="you"><b>まとめ</b>　'+
  'レヴィンの理論そのものは、80年以上たった今も心理学と組織論の土台として使われています。'+
  'いっぽうで、上の表のとおり<b>本診断の点数にあたる部分は本ツールの構成</b>です。'+
  '点数を「測定値」として受け取らず、'+
  '<b>自分を振り返るための問いかけ</b>として使ってください。</div>';
}
...
function showResult(force){
  ...
  if(!blocked) h+='<p class="noprint"><button class="btn ghost" id="toggleEx" style="padding:5px 12px;font-size:12.5px">解説をすべて閉じる</button></p>';
  SEC.title=h; h='';
  ...
  h+=ex("この章の図の読み方",
      ...
      '「どちら寄りか」という位置として示します。'+
      '項目名も「高い側の呼び名」ではなく<b>物差しそのものの名前</b>にしてあります。'+
      '片側の極をそのまま項目名にすると、反対側に寄っている人の行が'+
      '「高い水準を課す ／ 中点よりかなり下」のように自己矛盾してしまうためです</li></ul>'+
...

  {
    ...
    if(t) h+=you(t+'　これらの組み合わせが、以降のタイプ判定と行動の式の内容を決めています。');
  }
  ...
  /* 2 導出指標
     ...
     付けていたため、結論の章より長くなり、打ち手とも内容が重なっていた。
     中点から離れているものだけを解説し、残りは表にまとめる。 */
  if(mode!=="screening"){
    h+='<h3>組み合わせて見える6つの指標</h3>';
    h+=ex("この章は何のためにあるのか",
      '<p>前章の各項目は、単独で見ても「それで、どうすればよいのか」が出てきません。'+
      'この章は、いくつかの項目を組み合わせて<b>打ち手のもとになる形</b>に変換した数値です。</p>'+
      '<p>打ち手そのものは「まず何をするか」に書いています。'+
  ...
  if(cls){
    ...
    h+='<div class="card">'+
      ...
      '<dl class="kv" style="margin-top:12px"><dt>打ち手</dt><dd>'+td.rx+'</dd></dl></div>';
  ...
  }
  {
    ...
    if(watch.length){
      secAction+='<div class="card"><h3 style="margin-top:0">3か月後に見るもの</h3>'+
        '<p class="dim">全部を測り直す必要はありません。'+
        '上の打ち手に対応する項目だけを比べてください。'+
  ...
  if(sc.length){
    let secScene='<h2>場面別に見ると</h2>'+
      '<p class="dim">同じ結果でも、どの場面の話なのかが分からないと使えません。'+
      '日常の4つの場面に分けて書きます。'+
  ...
  let head3='';
  {
    /* 型名は直前のブロックで示しているので、ここでは人物像を1行で繰り返す */
    const t1 = cls&&TYPEDESC[cls.name]
      ? "<b>"+TYPEDESC[cls.name].nick+"</b>（"+cls.name+"）"+
        (cls.border? "。ただし分かれ目に近いので、決めつけずに読んでください":"")
      : null;
    ...
    head3='<div class="card" style="border-width:2px;border-color:var(--accent)">'+
      '<h2 style="margin-top:0;border:0;padding:0">ひとことでいうと</h2><dl class="kv">'+
      (t1? '<dt>あなたは</dt><dd><b>'+t1+'</b></dd>':'')+
      (t2? '<dt>効きどころ</dt><dd>'+t2+'</dd>':'')+
      (acts.length? '<dt>最初の一手</dt><dd><b>'+acts[0].t+'</b>'+
        '<span class="dim">　── 中身はすぐ下に</span></dd>':'')+
      '</dl><p class="dim" style="margin-bottom:0">'+
      'ここから下は、この3行の理由です。'+
      '時間がなければ「まず何をするか」だけ読んでも足ります。</p></div>';
  }

  /* ── 画面用の組み立て：結論 → 打ち手 → 理解 → 詳細データ ──
     測定の順（妥当性→素点→指標→類型）は分析する側の順序であって、
     読む側の順序ではない。読み手の問いは
     「自分はどういう人か」「だから何をすればいいか」「なぜそう言えるか」
     の順に来るので、その順に並べ替える。
  ...
  let screenBody = portraitBlock(cls,d) + head3 + SEC.action
    + SEC.scene + SEC.type + SEC.equation + SEC.force
    + SEC.profile
    + (va && va.verdict==="範囲内"
  ...
  screenBody += theorySources();
  screenBody += '<div class="note"><b>この結果について知っておいてほしいこと</b><br>'+
     'この診断は、正式に検証された心理検査ではありません。'+
     'レヴィンの考え方を借りて、自分を振り返るための道具です。<br>'+
     '中点スコアは、大勢の人を実際に測って作った基準ではなく、'+
     '「6段階の真ん中（3.50）を50とする」という決めごとから計算した仮の数です。<br>'+
     '病気の診断、向き不向きの判定、採用や人事評価には使わないでください。<br>'+
     'そして結果は、今の環境の中でのあなたを写したものです。'+
     '環境が変われば結果も変わります。'+
...
function portraitBlock(cls,d){
  ...
  return '<div class="card" style="border-width:2px;border-color:var(--accent)">'+
    ...
    'この診断は当たっていることを前提にしていません。'+
    '呼び名は、同じ人が2回受けても<b>7割ほどしか一致しません</b>'+
    '（測定誤差だけを与えた模擬計算による。上の2本の位置そのものは9割方一致します）。'+
    'これは連続した量を2つに切って呼び名をつける方式の限界で、'+
    'レヴィン自身も1931年に、二分法的な分類は連続的な段階づけに置き換えるべきだと書いています。<br>'+
    '心当たりがなければ、'+(other? '<b>'+other.nick+'（'+td.near+'）</b>':'ほかの型')+
    'の説明のほうが近いかもしれません。下の「あなたのタイプと、組むとよい相手」に4つとも載せています。<br>'+
    '<span class="dim">読むときは、「自分に当てはまるか」ではなく'+
    '「自分とまるで違う人にも当てはまってしまわないか」を見てください。'+
    'どちらにも当てはまる文なら、その文は何も言っていません。</span></div></div>';
}

/* ── 場面別に落とす ───────────────────────────────────
   打ち手だけでは「日常のどの瞬間の話か」が見えない。
...
function suggestions(s,d,order){
  /* ここは両極性の項目の「特徴」だけを載せる。
     具体的な打ち手は「まず何をするか」に一本化しているので繰り返さない。 */
