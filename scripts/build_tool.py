...
:root{
  ...
  /* 角丸・影。主要カードで共通のトークンを使い、以前の「総合判定ボックスだけ
     インラインで影がある」という非対称をなくす。 */
  --radius:10px; --radius-sm:6px;
  --shadow-card:0 1px 2px rgba(28,31,35,.04), 0 6px 20px rgba(28,31,35,.07);
  /* 数式・数値専用の書体。カスタムWebフォントは埋め込まない
...
.btn{display:inline-block;background:var(--accent);color:#fff;border:0;border-radius:var(--radius-sm);
  padding:11px 22px;font-size:14px;font-family:inherit;cursor:pointer;letter-spacing:.03em;
  box-shadow:0 1px 2px rgba(28,31,35,.12);transition:background-color .15s,box-shadow .15s,transform .1s}
.btn:hover{filter:brightness(.92);box-shadow:0 2px 8px rgba(28,31,35,.18)}
...
.btn:disabled{background:#c3c8ce;cursor:default;box-shadow:none}
...
.bar i{display:block;height:100%;background:var(--drive);width:0;transition:width .25s}
...
.idx-v{font-size:20px;font-weight:700;color:var(--accent);font-variant-numeric:tabular-nums;
  font-family:var(--font-eq)}
...
.sw>div{border:1px solid var(--line);border-radius:var(--radius-sm);padding:12px 14px;background:var(--card)}
...
ul.chk li{position:relative;padding:8px 10px 8px 34px;border:1px solid var(--line);
  border-radius:var(--radius-sm);margin-bottom:6px;background:var(--card);font-size:13.5px;line-height:1.7}
...
<div id="intro">
  ...
  <p class="lead">心理学者クルト・レヴィンのこの考え方をもとに、
...
function forcefield(s, rowH){
  ...
  g+='<text x="310" y="'+(H-12)+'" font-size="11" text-anchor="middle" fill="var(--faint)">今の状態</text>';
  ...
  drive.forEach((x,i)=>{ const y=42+i*rowH, len=Math.max(24,((x[1]-20)/60)*180);
    g+='<line x1="'+(305-len)+'" y1="'+y+'" x2="300" y2="'+y+'" stroke="var(--drive)" stroke-width="'+
      ...
      '<text x="'+(300-len-8)+'" y="'+(y+4)+'" font-size="11" text-anchor="end" fill="var(--ink)">'+
      x[0]+' <tspan fill="var(--faint)">'+x[1]+'</tspan></text>'; });
  hold.forEach((x,i)=>{ const y=42+i*rowH, len=Math.max(24,((x[1]-20)/60)*180);
    g+='<line x1="'+(315+len)+'" y1="'+y+'" x2="320" y2="'+y+'" stroke="var(--hold)" stroke-width="'+
      ...
      '<text x="'+(320+len+8)+'" y="'+(y+4)+'" font-size="11" fill="var(--ink)">'+
      x[0]+' <tspan fill="var(--faint)">'+x[1]+'</tspan></text>'; });
