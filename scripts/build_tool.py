...
function showResult(force){
  ...
  if(cls){
    ...
    h+='<p class="dim">ここから先は、ここまでの内容を4つの「型」としてラベル化した補足情報です。'+
  ...
  if(mode!=="screening"){
    ...
    h+=ex("なぜその打ち手なのかは、この式で決まっています",
      ...
      'このあとに出てくる「まず何をするか」の打ち手は、この式から導いています。'+
  ...
  let head3='';
  {
    ...
    head3='<div class="card" style="border-width:2px;border-color:var(--accent)">'+
      ...
      'ここまでの、位置・図・式・力の場の内容をひとことでまとめました。'+
      '時間がなければ、この下の「まず何をするか」だけ読んでも足ります。</p></div>';
  }

  /* ── 画面用の組み立て：位置 → 根拠 → まとめ → 打ち手 → 場面 → 型（補足） → 詳細データ ──
