...
function showResult(force){
  ...
  if(s.F1!=null){
    ...
    h+='<p class="dim">上の式は、行動がなぜ起こるかの一般的な見方でした。'+
       'ここではその見方を使って、<b>あなたが今どこで止まっているか</b>を図にします。</p>';
    h+=ex("なぜ今のままなのかを図で見る",
  ...
  if(sc.length){
    let secScene='<h2>場面別に見ると</h2>'+
      '<p class="dim">上の打ち手は項目ごとの話でした。ここでは同じ内容を、'+
  ...
  let head3='';
  {
    ...
    const t3 = (d.weak && CONFLICT_SCENE[d.weak])
      ...
      : "";
    const lead = t1
      ? "位置・図・式・力の場を総合すると、あなたは"+t1+"にあたります。"+t3
      : "";
